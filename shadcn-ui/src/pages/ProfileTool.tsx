...
function isGibberish(text: string) {
  ...
  const longRandomWord = words.some((word) => word.length > 18 && !/[aeiou]{2}/.test(word));
  return longRandomWord;
}
...
function buildAnswerFeedback(answer: string) {
  ...
  const hasAction =
    ...
    lower.includes("i helped") ||
    lower.includes("i managed") ||
    lower.includes("i organized") ||
    lower.includes("i built") ||
    lower.includes("i repaired") ||
  ...
  if (words.length < 35) {
    return {
      ...
      improved:
        "Try using this structure: Situation — what was happening. Action — what you personally did. Result — what changed because of your work.",
    };
  }
  ...
  if (!hasResult) {
    return {
      ...
      improved:
        "End with something like: “As a result, the work was completed on time, the customer was satisfied, or the process improved.”",
    };
  }
...
function FeedbackBox({ rating, feedback, improved }: { rating: string; feedback: string; improved: string }) {
  return (
...
function InterviewPrep() {
  ...
  const [review, setReview] = useState<ReturnType<typeof buildAnswerFeedback> | null>(null);
  ...
  return (
    <>
      <ToolHeader
        ...
        description="Practice mock interview questions and get feedback that explains what is strong, what is missing, and how to improve."
      />
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border bg-card p-6 shadow-sm">
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row">
            <Button onClick={() => setReview(buildAnswerFeedback(answer))}>Review My Answer</Button>
            <Button variant="outline" onClick={nextQuestion}>New Question</Button>
          </div>
        </div>

        <div className="rounded-2xl border bg-background p-6 shadow-sm">
          <h2 className="text-xl font-semibold text-foreground">Answer Review</h2>
...
function RolePlayPractice() {
  ...
  const [review, setReview] = useState<ReturnType<typeof buildRolePlayFeedback> | null>(null);
  ...
  return (
    <>
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border bg-card p-6 shadow-sm">
          <h2 className="text-xl font-semibold text-foreground">Workplace Scenario</h2>
          <p className="mt-4 rounded-xl border bg-background p-4 text-sm font-medium">
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row">
            <Button onClick={() => setReview(buildRolePlayFeedback(response))}>Review Response</Button>
            <Button variant="outline" onClick={nextScenario}>Try Another Scenario</Button>
          </div>
        </div>

        <div className="rounded-2xl border bg-background p-6 shadow-sm">
          <h2 className="text-xl font-semibold text-foreground">Workplace Feedback</h2>
          <p className="mt-3 text-sm text-muted-foreground">
  ...
  return (
    <>
      ...
      <section className="mt-8 rounded-2xl border bg-card p-6 shadow-sm">
        <h2 className="text-xl font-semibold text-foreground">Career Fit Quiz</h2>
                ...
                {item.options.map((option) => (
                  <label key={option.label} className="flex cursor-pointer gap-3 rounded-lg border bg-card p-3 text-sm">
                    <input
                      ...
                      onChange={() => setAnswers((current) => ({ ...current, [index]: option.value }))}
                    />
        ...
        <Button className="mt-6" onClick={getResult}>See My Result</Button>
...
function CareerPlan() {
  ...
  return (
    <>
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border bg-card p-6 shadow-sm">
          <h2 className="text-xl font-semibold text-foreground">Build Your Career Snapshot</h2>

          <div className="mt-5 space-y-4">
            <input value={targetRole} onChange={(e) => setTargetRole(e.target.value)} placeholder="Target job, client type, or opportunity" className="w-full rounded-xl border bg-background p-4 text-sm" />
            <textarea value={currentStrengths} onChange={(e) => setCurrentStrengths(e.target.value)} placeholder="Current strengths or experience you already have" className="w-full rounded-xl border bg-background p-4 text-sm" rows={3} />
            <textarea value={skillsToBuild} onChange={(e) => setSkillsToBuild(e.target.value)} placeholder="Skills you need to improve or prove" className="w-full rounded-xl border bg-background p-4 text-sm" rows={3} />
            <textarea value={trustTagsToEarn} onChange={(e) => setTrustTagsToEarn(e.target.value)} placeholder="TrustTags or proof you should build next" className="w-full rounded-xl border bg-background p-4 text-sm" rows={3} />
            <textarea value={nextStep} onChange={(e) => setNextStep(e.target.value)} placeholder="Your next action step" className="w-full rounded-xl border bg-background p-4 text-sm" rows={3} />
          </div>
        </div>
